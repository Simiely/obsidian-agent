...
3. 修改文档时调用 write_file 工具。**write_file 只暂存变更，用户确认后才生效**，请明确告知用户"已暂存，等待确认"。
...
class PendingStore:
    ...
    def stage(self, session_id: str, kind: str, path: str, content: str,
              backup_path: str, diff: str) -> PendingOp:
        op = PendingOp(
            op_id=f"op-{next(self._seq)}",
            kind=kind, path=path, content=content,
            backup_path=backup_path, diff=diff,
    ...
    def apply(self, session_id: str, op_id: str, deps: AgentDeps) -> dict:
        """确认后落盘：写入 + 增量索引。"""
    ...
    def discard(self, session_id: str, op_id: str) -> dict:
        op = self.get(session_id, op_id)
        if not op:
            raise SafetyError(f"操作不存在或已过期: {op_id}")
        if op.status != "pending":
...
class AgentService:
    ...
    def _build_agent(self, model: object | None = None) -> Agent:
        if model is None:
        ...
        agent = Agent(
            ...
            model_settings={"temperature": self.settings.llm_temperature,
        ...
        logger.info("Agent 构建完成 model=%s", model if isinstance(model, str) else type(model).__name__)
        return agent
    ...
    async def _tool_write_file(ctx: RunContext[AgentDeps], path: str, content: str) -> str:
        ...
        op = deps.pending.stage(
            deps.session_id, "write", path, content,
            check["backup_path"], check["diff"],
        )
        # 推送 tool 事件 → 前端确认框
        deps.events.put(("tool", {
            "name": "write_file",
            "opId": op.op_id,
            "path": path,
            "diff": check["diff"],
        ...
        return (f"写入操作 #{op.op_id} 已暂存（等待用户确认）：{path}\n"
                f"变更概览：{check['old_size']} → {check['new_size']} 字符，"
                f"备份: {check['backup_path']}")

    # ---------- 会话运行 ----------

    def run_chat(self, session_id: str, message: str) -> "queue.Queue[tuple[str, dict]]":
        ...
        deps = AgentDeps(
            vault=self.vault, index=self.index, backup=self.backup,
            settings=self.settings, pending=self.pending,
